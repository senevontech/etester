...
import { Activity, ArrowRight, BookOpen, ShieldCheck, UserCog } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
    ...
    const cards = [
        { label: 'User Management', value: String(orgMembers.length), icon: UserCog, onClick: () => navigate('/admin/users') },
        { label: 'Test Management', value: String(tests.length), icon: BookOpen, onClick: () => navigate('/admin/tests') },
        { label: 'Assignment Management', value: String(orgMembers.filter((member) => member.role === 'student').length), icon: ShieldCheck, onClick: () => navigate('/admin/assignments') },
        { label: 'Interview Management', value: 'Live', icon: Activity, onClick: () => navigate('/admin/interviews') },
    ];

    return (
        <RoleShell role="admin" activeKey="dashboard">
            <section className="anim-fade-up" style={{ marginBottom: '1.5rem' }}>
                <p className="t-micro" style={{ color: 'var(--text-muted)', marginBottom: '0.4rem' }}>Admin Dashboard</p>
                ...
                <p className="t-body" style={{ color: 'var(--text-muted)', maxWidth: '640px' }}>
                    Manage your team, tests, assignments, and interviews from one place. Use the sidebar to move between sections quickly.
                </p>
            </section>
