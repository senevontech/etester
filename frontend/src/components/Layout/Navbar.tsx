    ...
    return (
        ...
        <header style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, background: 'color-mix(in srgb, var(--bg) 20%, transparent)', backdropFilter: 'blur(20px) saturate(180%)', WebkitBackdropFilter: 'blur(20px) saturate(180%)', borderBottom: '1px solid color-mix(in srgb, var(--border) 40%, transparent)', transition: 'background 0.25s ease' }}>
            <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '56px' }}>
                {/* Logo */}
                <Link to={isAuthenticated ? (user?.role === 'admin' ? '/admin/dashboard' : user?.role === 'subadmin' ? '/subadmin/dashboard' : user?.role === 'superadmin' ? '/superadmin/dashboard' : '/dashboard') : '/'} style={{ display: 'flex', alignItems: 'center', textDecoration: 'none', flexShrink: 0 }}>
                    <img
                        src={logoSrc}
                        alt="Etester"
                ...
                <nav style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }} className="desktop-nav">
                    ...
                    {user?.role === 'subadmin' && (
                        <Link to="/subadmin/dashboard" style={{ padding: '0.375rem 0.75rem', borderRadius: '6px', fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-muted)', textDecoration: 'none', transition: 'all 0.15s ease', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                            <ShieldAlert size={14} /> Subadmin Dashboard
                        </Link>
                    )}
                    {user?.role === 'admin' && (
                        <>
                            <Link to="/admin/dashboard" style={{ padding: '0.375rem 0.75rem', borderRadius: '6px', fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-muted)', textDecoration: 'none', transition: 'all 0.15s ease', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                                <ShieldAlert size={14} /> Admin Dashboard
                            </Link>
                            <Link to="/admin/tests" style={{ padding: '0.375rem 0.75rem', borderRadius: '6px', fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-muted)', textDecoration: 'none', transition: 'all 0.15s ease', display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                                <ShieldAlert size={14} /> Test Management
                            </Link>
                        </>
                    )}
                </nav>
                            ...
                            {userMenuOpen && (
                                <>
                                    ...
                                    <div className="anim-fade-in" style={{ position: 'absolute', right: 0, top: 'calc(100% + 6px)', zIndex: 100, background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '10px', minWidth: '180px', boxShadow: 'var(--shadow-lg)', overflow: 'hidden' }}>
                                        ...
                                        {user.role === 'subadmin' && (
                                            <Link to="/subadmin/dashboard" onClick={() => setUserMenuOpen(false)}
                                        ...
                                        {user.role === 'admin' && (
                                            <>
                                                ...
                                                <Link to="/admin/tests" onClick={() => setUserMenuOpen(false)}
                                                    style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.625rem 1rem', fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-2)', textDecoration: 'none', transition: 'background 0.1s' }}
                                                    onMouseEnter={e => (e.currentTarget.style.background = 'var(--surface)')}
                                                    onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                                                >
                ...
                <div className="anim-fade-in" style={{ borderTop: '1px solid var(--border)', background: 'var(--bg)', padding: '0.75rem 1rem 1rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                    ...
                    {user?.role === 'subadmin' && (
                        <>
                            ...
                            <Link to="/subadmin/dashboard" onClick={() => setMobileOpen(false)} style={{ padding: '0.625rem 0.75rem', borderRadius: '8px', fontSize: '0.9rem', fontWeight: 700, color: 'var(--text-muted)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            <ShieldAlert size={14} /> Subadmin Dashboard
                            </Link>
                        </>
                    )}
                    {user?.role === 'admin' && (
                        <>
                            <Link to="/admin/dashboard" onClick={() => setMobileOpen(false)} style={{ padding: '0.625rem 0.75rem', borderRadius: '8px', fontSize: '0.9rem', fontWeight: 700, color: 'var(--text-muted)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                <ShieldAlert size={14} /> Admin Dashboard
                            </Link>
                            <Link to="/admin/tests" onClick={() => setMobileOpen(false)} style={{ padding: '0.625rem 0.75rem', borderRadius: '8px', fontSize: '0.9rem', fontWeight: 700, color: 'var(--text-muted)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                <ShieldAlert size={14} /> Test Management
                            </Link>
                        </>
                    )}
